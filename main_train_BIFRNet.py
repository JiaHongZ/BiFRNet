...
from data_loader import getImg as getImgtest
from data_loader import imgLoader as imgLoadertest
from data_loader import Imgset as Imgsettest
...
def test(mynet):
    ...
    for occ_level in occ_levels:
        ...
        for index, occ_type in enumerate(occ_types):
            # load images
            test_imgs, test_labels, masks = getImgtest('test', categories, dataset, data_path, categories_test, occ_level,
                                                   occ_type, bool_load_occ_mask=False)
            print('Total imgs for test of occ_level {} and occ_type {} '.format(occ_level, occ_type) + str(
                len(test_imgs)))
            # get image loader
            test_imgset = Imgsettest(test_imgs, masks, test_labels, imgLoadertest, bool_square_images=False)
...
def train_mynet(train_loader):
    ...
    # teacher.load_state_dict(torch.load(now_path + r'.\model_zoo\BIFRNet\VGG16\pascal3d+\best.pth'),strict=True)
    # teacher = teacher.module.hard_att.cuda()
    now_path = os.getcwd()
    teacher.load_state_dict(torch.load(now_path + r'.\model_zoo\BIFRNet\VGG16\pascal3d+\best.pth'),strict=True)
...
if __name__ == '__main__':
    ...
    # log path
    log_path = os.path.join(path, 'train_log_'+str(length)+'.log')
    print(log_path)
