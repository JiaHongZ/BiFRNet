...
def train_mynet(train_loader):
    ...
    mynet.load_state_dict(torch.load(path +'/final.pth'),strict=True)
    mynet = mynet.cuda()

    print('-----start train model------')
    # test(model)
    train_loader = DataLoader(dataset=train_imgset, batch_size=batch_size, shuffle=True)
    optimiter = torch.optim.Adam(mynet.parameters(),lr=0.00005,betas=(0.9, 0.999),eps=1e-8, weight_decay=0.0001)
    ######### Scheduler ###########
    warmup = True
    if warmup:
